...
async function onBtnAsync(i, env, w) {
  ...
  try {
    ...
    try { await follow({ content: `❌ خطأ: ${e.message}`, flags: 64 }); } catch(err) { console.error("FOLLOW:", err.message); }
  }
}
...
async function sendChartMsg(w, key) {
  ...
  if (msgId) {
    try {
      await fetch(`${w}/messages/${msgId}`, { method: "PATCH", headers: jH(), body: JSON.stringify({ components: [{ type: 1, components: [{ type: 2, style: 2, label: "🔄 تحديث", custom_id: `rc|${key}` }, { type: 2, style: 4, label: "🗑️ حذف", custom_id: `delmsg|${msgId}` }] }] }) });
...
function detailKB(k) {
  return [
    { type: 1, components: [ { type: 2, style: 2, label: "🔄 تحديث", custom_id: `rpa|${k}` }, { type: 2, style: 2, label: "📊 رسم بياني", custom_id: `c|${k}` }, { type: 2, style: 3, label: "🔔 تنبيه", custom_id: `al|${k}` } ]},
