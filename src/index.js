const HL="https://api.hyperliquid.xyz/info",API="https://discord.com/api/v10",QC="https://quickchart.io/chart";
const ASSETS={BTC:{sym:"BTC",ar:"بيتكوين",icon:"₿"},ETH:{sym:"ETH",ar:"إيثريوم",icon:"🔷"},SOL:{sym:"SOL",ar:"سولانا",icon:"🟣"},GOLD:{sym:"xyz:GOLD",ar:"الذهب",icon:"🟡"},SILVER:{sym:"xyz:SILVER",ar:"الفضة",icon:"⚪"},OIL:{sym:"xyz:CL",ar:"النفط",icon:"🛢"},US100:{sym:"xyz:XYZ100",ar:"ناسداك",icon:"📈"},SP500:{sym:"xyz:SP500",ar:"S&P 500",icon:"📊"}};
const ALIAS={btc:"BTC",bitcoin:"BTC",بيتكوين:"BTC",eth:"ETH",ethereum:"ETH",إيثريوم:"ETH",sol:"SOL",سولانا:"SOL",gold:"GOLD",xau:"GOLD",ذهب:"GOLD",silver:"SILVER",xag:"SILVER",فضة:"SILVER",oil:"OIL",cl:"OIL",wti:"OIL",نفط:"OIL",us100:"US100",nasdaq:"US100",nq:"US100",ناسداك:"US100",sp500:"SP500",es:"SP500",us500:"SP500",sp:"SP500"};
...
if(u.searchParams.get("debug")==="1"){const c={PK:!!env.DISCORD_PUBLIC_KEY,BT:!!env.DISCORD_BOT_TOKEN,AI:!!env.DISCORD_APP_ID,KV:!!env.DISCORD_KV};try{await env.DISCORD_KV.put("_t","1");await env.DISCORD_KV.delete("_t");c.KV_OK=true}catch(e){c.KV_OK=false;c.ERR=e.message}return new Response(JSON.stringify(c,null,2),{headers:{"Content-Type":"application/json"}});}
if(u.searchParams.get("alerts")==="1"){try{const chs=await getAllChannels(env);const out=[];for(const chId of chs){const a=await getChAlerts(env,chId);out.push({channel:chId,alerts:a.length,items:a})}return new Response(JSON.stringify(out,null,2),{headers:{"Content-Type":"application/json"}});}catch(e){return new Response(JSON.stringify({error:e.message}),{headers:{"Content-Type":"application/json"}})}}
if(u.searchParams.get("testalert")==="1"){try{const chId=u.searchParams.get("ch");if(!chId)return new Response("أضف ?ch=CHANNEL_ID",{headers:{"Content-Type":"text/plain"}});await sendMsg(env,chId,{title:"🔔 اختبار",color:0x00D278,description:"البوت يعمل!\nالتنبيهات تُرسل لهذه القناة.",footer:{text:fmtTs()},timestamp:new Date().toISOString()});return new Response("✅ تم الإرسال",{headers:{"Content-Type":"text/plain"}});}catch(e){return new Response(`❌ ${e.message}`,{headers:{"Content-Type":"text/plain"}});}}
...
}catch(e){console.error("ERR:",e.message);return new Response(JSON.stringify({error:e.message}),{status:500,headers:{"Content-Type":"application/json"}});}
...
function h2b(h){const b=new Uint8Array(h.length/2);for(let i=0;i<h.length;i+=2)b[i/2]=parseInt(h.substring(i+2,i+2),16);return b;}
...
const cond=tgt>cur?">":"<",dist=Math.abs((tgt-cur)/cur*100).toFixed(2);
...
if(before===filtered.length)return Response.json({type:4,data:{content:`📭 لا توجد تنبيهات لـ ${ASSETS[key].ar} في هذه القناة`,flags:64}});
...
const d=i.data.custom_id,H=jH(),chId=i.channel_id;
...
if(d.startsWith("rpa|")){const key=d.split("|")[1];const det=await fetchDetail(key);if(det.price)await patch({embeds:[priceEmbed(key,det)],components:detailKB(key)});return;}
if(d.startsWith("c|")){const key=d.split("|")[1],A=ASSETS[key];await patch({content:`⏳ جاري إنشاء رسم ${A.ar}...`,embeds:[],components:mainKB()});await sendChartMsg(w,key);return;}
if(d.startsWith("rc|")){await sendChartMsg(w,d.split("|")[1]);return;}
if(d.startsWith("delmsg|")){const msgId=d.split("|")[1];if(chId)await fetch(`${API}/channels/${chId}/messages/${msgId}`,{method:"DELETE",headers:botH(env)}).catch(()=>{});return;}
if(d.startsWith("del|")){
const aid=parseInt(d.split("|")[1]);const al=await getChAlerts(env,chId);await setChAlerts(env,chId,al.filter(a=>a.id!==aid));
await patch({content:"🗑️ تم حذف التنبيه",embeds:[],components:mainKB()});return;}
}catch(e){console.error("BTN:",e.message);try{await follow({content:`❌ خطأ: ${e.message}`,flags:64});}catch{};}
}
...
const cfg={type:"line",data:{labels:dL,datasets:[{data:dP,borderColor:clr,backgroundColor:fill,borderWidth:2,pointRadius:0,fill:true,tension:0.3}]},options:{plugins:{legend:{display:false},title:{display:true,text:`${A.ar}  ${up?"▲":"▼"} ${chg>=0?"+":""}${chg.toFixed(2)}%`,color:"#e0e0e0e0",font:{size:15,weight:"bold"}}},scales:{x:{ticks:{color:"#888",maxTicksLimit:8,maxRotation:0},grid:{color:"rgba(255,255,255,0.04)"}},y:{position:"right",ticks:{color:"#888",callback:v=>"$"+Number(v).toLocaleString("en-US",{minimumFractionDigits:0})},grid:{color:"rgba(255,255,255,0.06)"},min:+(minP*0.9997).toFixed(2),max:+(maxP*1.0003).toFixed(2)}},layout:{padding:{left:8,right:8,top:4,bottom:4}}}};
let imgBuf;
try{
